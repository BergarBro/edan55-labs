...
\emph{Please 
let us know if you detect any errors in the instructions.}
...
You would like to pick the post company that has the lowest estimated delivery time.
...
A road is described by 5 numbers, $u\, v\, t_{uv}\, p_{uv} \, p_{vu}$ where $u$ and $v$ are the ids of the intersections the road connects, 
...
while $p_{vu}$ is the probability of travelling from $v$ to $u$ if you are located at intersection $v$.

\medskip

All probabilities $p_{uv}$ where $u \neq H$ satisfy $0 < p_{uv} \leq 1$.

It is guaranteed that for each intersection the total probability of leaving that city is $1.0$,
except for the the intersection $H$, where the driver stops and delivers your package, 
hence here the probability of leaving is $0$.
...
The simulation picks a random road according to the probability distribution 
...
$[\ldots]$ happens when the original versions of the algorithms were run on the graph strange1.in. 
...
This is used as an exercise in identifying that a problem can be modelled as a Markov chain.
