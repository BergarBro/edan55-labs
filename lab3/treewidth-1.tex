...
\emph{Last time this exercise was tried, not all problems were resolved.}
...
%
You can also use the so-called \emph{nice tree decompositions}: 
the implementation becomes much simpler and not necessarily 
less efficient. See Section 7.3.1 of Cygan \emph{et al.}, 
%Fomin, Kowalik, Lokshtanov, Marx, Pilipczuk, Pilipczuk, and Saurabh, 
\emph{Parameterized algorithms} for the description of (a generalized version of) the 
algorithm.

The input files come in pairs with extension {\tt .td} and {\tt .gr}, respectively.
The format is described in {\tt data/README.md}.

You need to achieve a running time of $\exp(O(w))\operatorname{poly}(n)$; a straightforward 
implementation of the pseudo-code in either of the books will achieve that.
%, as analysed at the end of Section 10.4.


\paragraph{Some thoughts about implementation (from Thore Husfeldt).}
